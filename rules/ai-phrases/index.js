...
const japanesePhrases = [
  ...
  [/頑健(?:に|な)/g, "「頑健に」という評価語を避け、具体的にどの程度・どう安定しているかを書く"],
  [/本質的に/g, "「本質的に」という強調語を避け、何がどうであるかを直接書く"],
  [/画定する/g, "「画定する」という硬い言い回しを避け、「示す」「絞り込む」等の平易な動詞にする"],
  [/直接判別/g, "「直接」という修飾が必要か確認し、不要なら削って「判別」とする"],
  [/速度不変表現/g, "「速度不変表現」という独自の複合語を避け、何を指すか平易に言い換える"]
];
